...
import { queryObsFromFbRef, getObsForDoc, readDoc } from "./readerFe"
...
const serverGetters = collectionNames.reduce(
  (acc, collectionName) => {
    return {
      ...acc,
  ...
  {} as {
    [key in keyof CollectionNameToModelType]: (
      id: string
    ) => Promise<CollectionNameToModelType[key]>
  }
)
